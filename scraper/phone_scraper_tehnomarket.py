...
BASE_URL = "https://www.tehnomarket.com.mk"
CATEGORY_URL = f"{BASE_URL}/category/4109/mobilni-telefoni"
IMAGE_SELECTOR = ".pbox-thumbnail figure"

# tehnomarket blocks hotlinked images, so we download them locally and serve them
# from our own backend instead of linking directly to tehnomarket's CDN.
...
def scrape_all_phones():
    ...
    while True:
        ...
        if not loaded:
            print(f"No cards on page {page}, stopping.")
            break
        # Force the whole page to render so lazy-loaded (background-image) product
        # photos actually populate before we start reading each card individually.
            ...
            try:
                ...
                try:
                    ...
                    imageUrl = download_image_locally(
                        remote_image_url, "tehnomarket", BASE_URL, IMAGES_DIR, BACKEND_IMAGE_BASE_URL
                    )
                except:
                    imageUrl = None
