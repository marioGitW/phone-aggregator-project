...
def clean_price(raw_price_text):
    ...
    if not raw_text:
        logger.debug("[clean_price] raw='' -> 0")
        return 0

    candidate = re.sub(r"[^\d,.]", "", raw_text.replace("\xa0", " "))
    if not candidate:
        logger.debug(f"[clean_price] raw={raw_text!r} -> 0")
        return 0
    ...
    logger.debug(f"[clean_price] raw={raw_text!r} -> {value}")
    return value
...
def _build_local_image_filename(source, image_url):
    """Build a safe, unique local filename for a downloaded product image."""
    basename = image_url.split("?")[0].rstrip("/").split("/")[-1]
    basename = re.sub(r"[^a-zA-Z0-9._-]", "_", basename)
    if not basename or "." not in basename:
        basename = (basename or "image") + ".jpg"
    return f"{source}_{basename}"


def download_image_locally(image_url, source, referer, images_dir, backend_image_base_url):
    """
    Download a remote product image to a local folder (to work around sites that
    block hotlinking / cross-origin image loading), and return the URL that our
    OWN backend will serve it from, instead of the original remote URL.

    image_url: the remote image URL found by extract_image_url()
    source: site name, e.g. "anhoch" (used as filename prefix so sources never collide)
    referer: the site's own base URL, sent as the Referer header to satisfy hotlink checks
    images_dir: local folder to save the file into (must match what the backend serves)
    backend_image_base_url: e.g. "http://localhost:8083/images" - prefix for the returned URL
    """
    if not image_url:
        return None

    try:
        filename = _build_local_image_filename(source, image_url)
        ...
        return f"{backend_image_base_url}/{filename}"

    except Exception as e:
        logger.warning(f"[download_image_locally] failed for {image_url!r}: {e}")
        return None